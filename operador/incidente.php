...
if (isset($_GET['call_id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        // Update call status to "En curso"
...
if (isset($_GET['lat']) && isset($_GET['lng']) && !$edicion) {
    ...
    // Create empty incident with just location data
    $incidente = [
        'latitud' => $lat,
        'longitud' => $lng,
    ...
    $edicion = false; // This is a new incident from a call
}
...
<script>
    // Initialize map with possible pre-filled location (NEW CODE)
    ...
    // KEEP ALL YOUR EXISTING MARKER AND CLICK HANDLING CODE BELOW
    let marker = null;
